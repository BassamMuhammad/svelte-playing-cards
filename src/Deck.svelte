<script lang="ts">
  import type { CardType, Suit, ValueWithoutJoker } from "./global";
   ...
   */
  export let onDblClick: svelte.JSX.MouseEventHandler<HTMLElement> = (
    event
  ) => {
    shuffle();
  };
   ...
   * {duration: milliseconds to complete transition, easing: easing function, delay: milliseconds after which transition starts}
   ...
   *  cardProps = deckComp.drawTopCard(x,y)[2]
   * }}>
   * {#if cardProps}
  ...
  export const drawTopCard = (
    ...
    if (targetX && targetY) {
      renderedCard.transitionToTarget(targetX, targetY, options);
   ...
   * <div on:click={(e)=>{
   *  const { x, y } = e.currentTarget.getBoundingClientRect();
   *  cardsProps = cardProps.concat(deckComp.drawCards(5,x,y)[2])
   * }}>
  ...
  export const drawCards = (
    ...
    for (let i = 0; i < numOfCards; i++) {
      ...
      renderedDrawnCardsProps.push(drawn[2]);
    }
    let i = 0;
    const duration = 2000;
    renderedDrawnCards[i++].transitionToTarget(targetX, targetY, options);
    const interval = setInterval(() => {
      renderedDrawnCards[i++].transitionToTarget(targetX, targetY, options);
    }, duration);
    setTimeout(() => {
      clearInterval(interval);
    }, duration * renderedDrawnCards.length - 1);
  ...
  const shuffleTransition = (
    ...
    if (((2 * offset) / increment) * ms > repeatAfter)
      throw new Error(
        "does not satisfy ((2 * offset) / increment) * ms <= repeatAfter"
      );
    ...
    return new Card({
      ...
      props: {
        ...
        topPosition: (topPosition = `${
          parseFloat(topPosition.slice(0, topPosition.length - 2)) +
          topPositionOffset
        }px`),
        leftPosition: (leftPosition = `${
          parseFloat(leftPosition.slice(0, leftPosition.length - 2)) +
          leftPositionOffset
        }px`),
      },
    });
  ...
  const getFrontShowingCardIndex = (card: CardType) => {
    return frontShowingCards.findIndex(
      (frontShowingCard) => frontShowingCard === card
    );
  };
  ...
  const getCardCustomFrontComp = (card: CardType) => {
    const cardCustomFront = cardCustomFronts.find(
      (cardCustomFront) => cardCustomFront[0] === card
    );
...
<div
  ...
  style={`position:relative;width:${
    parseFloat(cardWidth.slice(0, cardWidth.length - 2)) +
    leftPositionOffset * deck.length
  }px;height:${
    parseFloat(cardHeight.slice(0, cardHeight.length - 2)) +
    topPositionOffset * deck.length
  }px`}
/>
