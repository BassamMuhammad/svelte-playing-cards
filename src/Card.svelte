<script lang="ts">
  ...
  /**top position (in case of position = absolute) @default 0px*/
  export let topPosition = "0px";
  /**left position (in case of position = absolute) @default 0px*/
  ...
  let srcBack = `./icons/cards/card-back.svg`;
   ...
   * @param options  options to control transition {duration: milliseconds to complete transition, easing: easing function, delay: milliseconds after which transition starts}
   *
  ...
  /**
   * get position of card (position refers to css top and left)
   * @returns [cssTop, cssLeft]
   */
  export const getPosition = (): [string, string] => [
    topPosition,
    leftPosition,
  ];
  /**
  ...
  export const shufflingTransition = (
    ...
    const interval = setInterval(() => {
      ...
      rootEle.style.transform = `translate${axis}(${(translate +=
        increment)}px)`;
    }, ms);
    setTimeout(() => {
